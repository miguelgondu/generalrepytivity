...
from generalrepytivity.utils import _dict_completer
import sympy
...
def test_dict_completer_for_tensor_1():
    dict_of_values = {(1, (0, 1)): 1, ((2,), (0, 1)): 2, (0, (1, 1)): 3, (3, (0, 0)): 4}
    basis = [t, x, y, z]
...
def test_return_value_one_multiindex():
    dict_of_values = {((0, 0, 0), ()): 1, ((0, 0, 1), ()): 2, ((0, 1, 0), ()): 3}
    basis = [t, x, y, z]
...
def test_wrong_dict_in_creation():
    _type = (2, 2)
    basis = [t, x, y, z]
    dict_of_values = {((0, 0), (1, 1)): 3, ((0,), (1, 2)): -1}
    try:
        tensor = gr.Tensor(basis, _type, dict_of_values)
    except ValueError:
        assert True
...
def test_index_contraction1():
    ...
    other_dict_of_values = {
        (a, b): sum([(sum(a) + r) / (sum(b) + r + 1) for r in range(4)])
        for a in other_indices
...
def test_index_contraction2():
    ...
    dict_of_values = {
        (a, b): 2 ** a[0] * 3 ** a[1] * 5 ** b[0] for a in c_indices for b in ct_indices
    }
    tensor = gr.Tensor(basis, _type, dict_of_values)
    contracted_tensor_1 = gr.contract_indices(tensor, 1, 0)

    dict_of_values2 = {
        ((a,), ()): sum([2 ** a * 3 ** r * 5 ** r for r in range(dim)])
        for a in range(dim)
    }
...
def test_lower_index1():
    ...
    dict_of_values_2 = {
        ...
        for a in ct_indices_2
        for b in c_indices_2
    }
...
def test_raise_index1():
    ...
    dict_of_values_2 = {
        (a, b): sum([inv_metric[r, a[2]] * tensor[(a[0], a[1]), r] for r in range(dim)])
        for a in c_indices_2
        for b in ct_indices_2
    }
...
def test_subs_in_tensor1():
    _type = (1, 1)
    s = sympy.Symbol("s")
    dict_of_values = {((1,), (0,)): s ** 2, ((1,), (1,)): s ** 3, ((0,), (1,)): 4}
    basis = [t, x, y, z]
    tensor_1 = gr.Tensor(basis, _type, dict_of_values)
    dict_of_values2 = {((1,), (0,)): 9, ((1,), (1,)): 27, ((0,), (1,)): 4}
...
def test_christoffel_symbols1_godel():
    ...
    christoffel_symbols_2 = _dict_completer(christoffel_symbols_2, 1, 2, 4)
